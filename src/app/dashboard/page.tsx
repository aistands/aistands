            ...
            {[
              ...
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr', borderBottom: i < 4 ? '1px solid var(--border)' : 'none', background: row.hdr ? 'var(--navy)' : 'transparent' }}>
                ...
                {[row.so, row.bsol, row.acc, row.astm].map((cell, j) => (
                  <div key={j} style={{ padding: '13px 20px', fontSize: '13px', color: row.hdr ? (j === 0 ? '#fff' : 'rgba(255,255,255,0.35)') : (j === 0 ? 'var(--orange)' : 'var(--text-muted)'), background: j === 0 && !row.hdr ? 'rgba(232,99,26,0.04)' : j === 0 && row.hdr ? 'rgba(232,99,26,0.15)' : 'transparent', fontFamily: row.hdr && j > 0 ? 'Epilogue, sans-serif' : 'inherit', fontWeight: row.hdr && j > 0 ? 700 : 'inherit', textTransform: row.hdr && j > 0 ? 'uppercase' as const : 'none' as const, fontSize: row.hdr && j > 0 ? '10px' : '13px', letterSpacing: row.hdr && j > 0 ? '0.08em' : 0 }}>
