  ...
  if (done) return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:'#0B1E3E'}}>
      <div className="text-center max-w-sm">
        ...
        <p className="text-sm text-slate-ai leading-relaxed">We've sent a confirmation link to <strong className="text-white">{email}</strong>. Click it to activate your account and get started.</p>
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:'#0B1E3E'}}>
      ...
      <div className="w-full max-w-[420px] relative">
        <div className="text-center mb-8">
          <Logo size="md" />
          <p className="text-sm text-slate-ai mt-2">Your AI workspace for standards</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[['5','Free queries'],['1','Free project'],['∞','Standards supported']].map(([val,label]) => (
            <div key={label} className="card rounded-xl p-3 text-center">
              ...
              <div className="text-[11px] text-slate-ai mt-0.5">{label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card p-8 rounded-2xl" style={{background:'#132952'}}>
          <form onSubmit={handleSignup} className="flex flex-col gap-5">
            ...
            <p className="text-[11px] text-slate-ai text-center leading-relaxed">
              By signing up you agree to our{' '}
              <Link href="/terms" className="text-electric-bright hover:underline">Terms</Link> and{' '}
              <Link href="/privacy" className="text-electric-bright hover:underline">Privacy Policy</Link>
            </p>
          </form>
          ...
          <div className="mt-6 text-center">
            ...
            <p className="text-sm text-slate-ai">
              ...
              <Link href="/auth/login" className="text-electric-bright hover:underline font-medium">Sign in</Link>
            </p>
          </div>
        </div>
      </div>
