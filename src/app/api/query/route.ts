...
function extractRelevantChunks(documentText: string, question: string, maxChars: number = 8000): string {
  ...
  if (result.length < 300) {
    result = documentText.slice(0, 6000)
  }
...
You are provided with the most relevant sections from a standards document.
Answer clearly and accurately in plain English. Reference clause numbers where relevant.
Be precise — compliance professionals rely on your answers.
If the specific section asked about is not in the provided text, say so clearly.
If using web sources, label what comes from the document vs the web.`

    let answer = ''
    let needsTextSave = false

    if (project.document_text && project.document_text.length > 500) {
      // Smart chunking — only send relevant sections, not whole document
      const relevantText = extractRelevantChunks(project.document_text, question)

      const userMessage = useWebSearch
        ? `Relevant document sections:\n<document>\n${relevantText}\n</document>\n\nAlso search the web for supporting guidance. Label sources.\n\nQuestion: ${question}`
        : `Relevant document sections:\n\n<document>\n${relevantText}\n</document>\n\nQuestion: ${question}`
      ...
      // No saved text yet — send PDF directly for this query only
      needsTextSave = true
    ...
    if (err.message?.includes('rate_limit')) {
      return NextResponse.json({ answer: 'The AI is busy — please wait 30 seconds and try again.' })
    }
